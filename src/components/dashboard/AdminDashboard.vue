<template>
  <div class="admin-dashboard space-y-8 pb-12">
    ...
    <section>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="bg-white dark:bg-slate-900 p-6 rounded-xl border border-primary/10 shadow-sm relative overflow-hidden group">
          ...
          <p class="text-sm font-medium text-slate-500 mb-1 uppercase tracking-wider">Main Balance</p>
          <div v-if="isLoading" class="h-8 w-32 bg-slate-100 dark:bg-slate-800 animate-pulse rounded mb-2"></div>
          <h4 v-else class="text-2xl font-bold mb-2">Rp {{ formatNumber(mainBalance) }}</h4>
          <div class="flex items-center gap-2">
            <span class="text-emerald-500 flex items-center text-xs font-bold bg-emerald-50 dark:bg-emerald-900/20 px-1.5 py-0.5 rounded">
              <span class="material-symbols-outlined text-xs mr-1">public</span> Global Kas
            </span>
            <span class="text-slate-400 text-[10px]">Community Total</span>
          </div>
        </div>

        <div class="bg-white dark:bg-slate-900 p-6 rounded-xl border border-primary/10 shadow-sm relative overflow-hidden group ring-2 ring-primary ring-offset-2 ring-offset-transparent dark:ring-offset-slate-900">
          <div class="absolute top-0 right-0 p-4 opacity-10 text-primary group-hover:scale-110 transition-transform">
            <span class="material-symbols-outlined text-6xl">build_circle</span>
          </div>
          <div class="flex justify-between items-start mb-1">
            <p class="text-sm font-medium text-slate-500 uppercase tracking-wider">Sinking Fund</p>
          </div>
          <div v-if="isLoading" class="h-8 w-32 bg-slate-100 dark:bg-slate-800 animate-pulse rounded mb-2"></div>
          <h4 v-else class="text-2xl font-bold mb-2 text-primary">Rp {{ formatNumber(sinkingFund) }}</h4>
          <div class="flex items-center gap-2">
            <span class="text-primary flex items-center text-xs font-bold">
              Iuran Lainnya
            </span>
            <span class="text-slate-400 text-[10px] hidden sm:block">Aggregated</span>
          </div>
        </div>
        ...
        <div class="bg-white dark:bg-slate-900 p-6 rounded-xl border border-primary/10 shadow-sm relative overflow-hidden group">
          ...
          <p class="text-sm font-medium text-slate-500 mb-1 uppercase tracking-wider">Other Income</p>
          <div v-if="isLoading" class="h-8 w-32 bg-slate-100 dark:bg-slate-800 animate-pulse rounded mb-2"></div>
          <h4 v-else class="text-2xl font-bold mb-2">Rp {{ formatNumber(otherIncome) }}</h4>
          <div class="flex items-center gap-2">
    ...
    <section class="grid grid-cols-1 lg:grid-cols-2 gap-8">
      <div class="bg-white dark:bg-slate-900 p-8 rounded-xl border border-primary/10 shadow-sm">
        ...
        <div class="h-64 flex flex-col gap-4">
          ...
          <div class="flex justify-between text-[10px] font-bold text-slate-400 tracking-tighter uppercase px-1">
      ...
      <div class="grid grid-cols-2 gap-4">
        <!-- MTD Income -->
        <div class="bg-white dark:bg-slate-900 p-6 text-center md:text-left rounded-xl border border-primary/10 shadow-sm flex flex-col justify-between">
          <p class="text-xs font-bold text-slate-400 uppercase">MTD Income</p>
          <div v-if="isLoading" class="h-6 w-24 bg-slate-100 dark:bg-slate-800 animate-pulse rounded mt-2 mx-auto md:mx-0"></div>
          <h5 v-else class="text-lg md:text-xl font-bold mt-2 text-emerald-600 dark:text-emerald-400">Rp {{ formatNumber(mtdIncome) }}</h5>
          <p class="text-[10px] text-slate-400 font-medium mt-1">Month to Date</p>
        </div>
        <!-- MTD Expense -->
        <div class="bg-white dark:bg-slate-900 p-6 text-center md:text-left rounded-xl border border-primary/10 shadow-sm flex flex-col justify-between">
          <p class="text-xs font-bold text-slate-400 uppercase">MTD Expense</p>
          <div v-if="isLoading" class="h-6 w-24 bg-slate-100 dark:bg-slate-800 animate-pulse rounded mt-2 mx-auto md:mx-0"></div>
          <h5 v-else class="text-lg md:text-xl font-bold mt-2 text-rose-500">Rp {{ formatNumber(mtdExpense) }}</h5>
          <p class="text-[10px] text-slate-400 font-medium mt-1">Month to Date</p>
        </div>
        <!-- YTD Income -->
        <div class="bg-white dark:bg-slate-900 p-6 text-center md:text-left rounded-xl border border-primary/10 shadow-sm flex flex-col justify-between">
          ...
          <div v-if="isLoading" class="h-6 w-24 bg-slate-100 dark:bg-slate-800 animate-pulse rounded mt-2 mx-auto md:mx-0"></div>
          <h5 v-else class="text-lg md:text-xl font-bold mt-2 text-emerald-600 dark:text-emerald-400">Rp {{ formatNumber(ytdIncome) }}</h5>
          <p class="text-[10px] text-slate-400 font-medium mt-1">Year to Date (Current Year)</p>
        </div>
        <!-- YTD Expense -->
        <div class="bg-white dark:bg-slate-900 p-6 text-center md:text-left rounded-xl border border-primary/10 shadow-sm flex flex-col justify-between">
          <p class="text-xs font-bold text-slate-400 uppercase">YTD Expense</p>
          <div v-if="isLoading" class="h-6 w-24 bg-slate-100 dark:bg-slate-800 animate-pulse rounded mt-2 mx-auto md:mx-0"></div>
          <h5 v-else class="text-lg md:text-xl font-bold mt-2 text-rose-500">Rp {{ formatNumber(ytdExpense) }}</h5>
          <p class="text-[10px] text-slate-400 font-medium mt-1">Year to Date (Current Year)</p>
        </div>
      </div>
    </section>
    ...
    <section>
      <div class="bg-white dark:bg-slate-900 rounded-xl border border-primary/10 shadow-sm overflow-hidden">
        ...
        <div v-if="isLoading" class="p-12 flex justify-center"><span class="material-symbols-outlined animate-spin text-3xl text-primary">refresh</span></div>
        <div v-else-if="recentTransactions.length === 0" class="p-12 text-center text-slate-400 font-medium">No recent transactions.</div>
        <div v-else class="overflow-x-auto">
          <table class="w-full text-left text-xs border-collapse">
            <thead>
              <tr class="bg-slate-50 dark:bg-slate-800/50">
                <th class="p-4 font-bold text-slate-400 uppercase tracking-wider">Date</th>
                <th class="p-4 font-bold text-slate-400 uppercase tracking-wider">Unit / Recipient</th>
                <th class="p-4 font-bold text-slate-400 uppercase tracking-wider">Description</th>
                <th class="p-4 font-bold text-slate-400 uppercase tracking-wider text-right">Amount</th>
                <th class="p-4 font-bold text-slate-400 uppercase tracking-wider text-center">Type</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
              <tr v-for="tx in recentTransactions" :key="tx.id" class="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-all">
                <td class="p-4 whitespace-nowrap">{{ formatDate(tx.transaction_date) }}</td>
                <td class="p-4 font-bold text-primary">{{ tx.recipient || (tx.description?.includes('-') ? tx.description.split('-')[0].trim() : 'System') }}</td>
                <td class="p-4 font-medium max-w-xs truncate" :title="tx.description">{{ tx.description || (tx.type === 'deposit' ? 'Income' : 'Expense') }}</td>
                <td class="p-4 text-right font-bold" :class="tx.type === 'deposit' ? 'text-emerald-500' : 'text-rose-500'">
                  {{ tx.type === 'deposit' ? '+' : '-' }} Rp {{ formatNumber(Math.abs(tx.amount)) }}
                </td>
                <td class="p-4 text-center">
                  <span v-if="tx.type === 'deposit'" class="px-2 py-0.5 bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400 rounded text-[10px] font-bold">INCOME</span>
                  <span v-else class="px-2 py-0.5 bg-rose-100 text-rose-700 dark:bg-rose-900/30 dark:text-rose-400 rounded text-[10px] font-bold">EXPENSE</span>
                </td>
              </tr>
            </tbody>
          </table>
...
onMounted(async () => {
  ...
  try {
    ...
    if (deposits) {
      // Sinking Fund = Anything labeled "Iuran Lainnya"
      sinkingFund.value = deposits
        .filter(d => d.description && d.description.includes('Iuran Lainnya'))
        .reduce((sum, d) => sum + (d.amount || 0), 0)
      
      // Other Income = Anything that is NOT IPL, THR, or Iuran Lainnya
      otherIncome.value = deposits
        .filter(d => {
          const desc = d.description || ''
          return !desc.includes('IPL') && !desc.includes('THR') && !desc.includes('Iuran Lainnya')
        })
        .reduce((sum, d) => sum + (d.amount || 0), 0)
    }
    ...
    if (performanceRes.success) {
      ...
      // Calculate a reasonable max for scaling
      const allValues = performanceRes.data.flatMap(d => [d.income, d.expense])
      const peak = Math.max(...allValues, 1000000)
      maxChartValue.value = peak * 1.2 // Add 20% headroom
    }
  } catch (err) {
