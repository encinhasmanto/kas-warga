...
// /**
//  * Transforms a static Bank Mandiri QRIS payload into a Dynamic one.
...
//   if (!amount || amount <= 0) return staticPayload;

//   // 1. Point of Initiation Surgery (Tag 01)
//   // Modifying '010211' (Static) to '010212' (Dynamic) ensures banking app will read the provided amount.
//   let dynamicBase = staticPayload.replace('010211', '010212');

//   // 2. Clear Existing CRC (Tag 63)
//   // The standard implies tag 63 is exactly 6304 + 4 character CRC. 
//   let cleanPayload = dynamicBase;
//   if (cleanPayload.slice(-8, -4) === '6304') {
//     cleanPayload = cleanPayload.slice(0, -8);
//   } else {
//     // Basic fallback if structure differs slightly based on regex.
//     const lastIdx = cleanPayload.lastIndexOf('6304');
//     if (lastIdx !== -1) {
//       cleanPayload = cleanPayload.slice(0, lastIdx);
//     }
//   }

//   // 3. Construct Amount Tag (Tag 54)
//   // Ensures length is always 2 digits and float has no decimal part.
...
//   // 4. Inject Tag 54 before Tag 58 (5802ID) 
//   // It is an EMVCo convention & safe spot for amounts in QR string structuring.
//   let payloadToSeal = '';
//   if (cleanPayload.includes('5802ID')) {
//     payloadToSeal = cleanPayload.replace('5802ID', `${tag54}5802ID`);
//   } else {
//     payloadToSeal = cleanPayload + tag54; // default fall if 5802ID is missing
//   }

//   // Add back the 6304 standard before sealing with CRC
//   payloadToSeal += '6304';

//   // 5. Final Step: Seal with new robust CRC Algorithm
