    ...
    <div class="w-full h-screen md:h-auto md:max-w-6xl md:bg-white md:dark:bg-slate-900 md:shadow-2xl md:rounded-3xl overflow-hidden flex flex-col md:flex-row border-0 md:border border-slate-200 dark:border-slate-800 relative z-10">
      ...
      <div class="w-full md:w-[480px] lg:w-[540px] bg-white dark:bg-slate-900 flex flex-col justify-center h-screen md:h-auto overflow-y-auto no-scrollbar relative z-10">
        ...
        <form class="px-8 md:px-12 pb-8 space-y-6" @submit.prevent="handleLogin">
          ...
          <div class="admin-field-container" :class="{ 'visible': selectedUnit === 'Admin' }">
        ...
        <div class="bg-slate-50 dark:bg-slate-800/50 p-6 md:px-12 border-t border-slate-200 dark:border-slate-800 mt-auto">
          ...
          <div class="grid grid-cols-3 gap-2">
            <button @click="enterGuestMode('Resident')" class="flex flex-col items-center justify-center gap-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 py-2.5 px-2 rounded-lg text-[11px] font-medium hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
              <span class="material-symbols-outlined text-base text-primary">person_outline</span>
              Resident
            </button>
            <button @click="enterGuestMode('Admin')" class="flex flex-col items-center justify-center gap-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 py-2.5 px-2 rounded-lg text-[11px] font-medium hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
              <span class="material-symbols-outlined text-base text-primary">admin_panel_settings</span>
              Admin
            </button>
            <button @click="enterGuestMode('Super Admin')" class="flex flex-col items-center justify-center gap-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 py-2.5 px-2 rounded-lg text-[11px] font-medium hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
              <span class="material-symbols-outlined text-base text-primary">shield_person</span>
              Super
            </button>
          </div>
        </div>
...
function handleForgotPin() {
  if (!selectedUnit.value || selectedUnit.value === 'Admin') {
    alert("Mohon pilih Unit tempat tinggal Bapak/Ibu dahulu sebelum menuju Forget PIN.")
  } else {
    const text = encodeURIComponent(`${selectedUnit.value} lupa PIN. Tolong dibantu diinfokan untuk akses PINnya. Terima kasih.`)
