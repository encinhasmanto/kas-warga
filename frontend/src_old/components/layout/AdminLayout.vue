    ...
    >
      ...
      <nav class="flex-1 px-4 py-4 space-y-1 overflow-y-auto no-scrollbar">
        <div v-for="section in menuSections" :key="section.title">
          ...
          <!-- Regular menu items (non-settings) -->
          <router-link
        ...
        >
          ...
          <div class="flex-1 overflow-hidden">
            ...
            <p class="text-[10px] text-slate-500 truncate">
              {{ session?.username || session?.unitCode || "Active Session" }}
            </p>
          </div>
          <button
            @click="handleLogout"
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
      ...
      >
        <div class="flex items-center gap-4">
          ...
          <div
            class="hidden md:flex items-center bg-slate-100 dark:bg-slate-800 rounded-lg px-3 py-1.5 gap-2 border border-transparent focus-within:border-primary/30 focus-within:bg-white dark:focus-within:bg-slate-900 transition-all"
          >
  ...
  return [
    {
      ...
      items: [
        ...
        { name: "bulletin", icon: "campaign", label: "Bulletin Board" },
      ],
    },
    {
      title: "Operations",
      items:
        session.value?.role === "Super Admin"
          ? [
              {
                name: "transactions",
                icon: "swap_horiz",
