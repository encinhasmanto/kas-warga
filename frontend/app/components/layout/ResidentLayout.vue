    ...
    <aside class="hidden md:flex w-64 bg-white dark:bg-slate-900 border-r border-primary/10 flex-col shrink-0">
      <div class="p-6 flex items-center gap-3">
        <div class="w-10 h-10 bg-primary/20 rounded-xl flex items-center justify-center text-primary shadow-sm overflow-hidden bg-slate-100 dark:bg-slate-800">
          <img 
            :src="avatarUrl || `https://api.dicebear.com/7.x/avataaars/svg?seed=${displayName || 'Resident'}`" 
            alt="Profile" 
            class="object-cover size-full"
          />
        </div>
        <div>
          <h1 class="font-bold text-lg leading-none truncate w-32">{{ displayName || 'Resident' }}</h1>
          <p class="text-xs text-slate-500 font-medium mt-1">Unit {{ unitCode || 'A1' }}</p>
        </div>
      </div>
      ...
      <div class="p-4 border-t border-primary/10">
        <div class="flex items-center gap-3 p-2 bg-slate-50 dark:bg-slate-800/50 rounded-xl">
          ...
          <button @click="logout" class="text-slate-400 hover:text-rose-500 transition-colors p-1">
    ...
    <div class="flex-1 flex flex-col overflow-hidden relative">
      ...
      <header class="flex items-center bg-white/90 md:bg-white dark:bg-slate-900/90 md:dark:bg-slate-900 backdrop-blur-md md:backdrop-blur-none p-4 md:px-8 border-b border-slate-200 dark:border-slate-800 justify-between shrink-0 z-20">
        ...
        <div class="flex items-center justify-end relative">
          ...
          <button @click="logout" class="md:hidden relative w-10 h-10 flex items-center justify-center rounded-full md:rounded-lg text-slate-500 hover:bg-rose-50 hover:text-rose-500 transition-all ml-1">
          ...
          <div class="hidden lg:text-right lg:block">
            ...
            >
              ...
              ></span> 
              {{ systemStatus === 'connected' ? 'Active' : systemStatus === 'limited' ? 'Limited' : 'Offline' }}
            </p>
          </div>
        </div>
      </header>
    ...
    <nav class="md:hidden fixed bottom-0 left-0 right-0 z-50 flex h-auto w-full flex-col bg-white/95 dark:bg-background-dark/95 backdrop-blur-md border-t border-slate-200 dark:border-slate-800 safe-bottom shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
      <div class="flex gap-2 px-4 pb-6 pt-3 max-w-xl mx-auto w-full">
        <NuxtLink 
          ...
          :class="[route.path.includes(item.path) ? 'text-primary' : 'text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300']"
        >
...
const { displayName, unitCode, avatarUrl, logout, session } = useAuth()
...
const navItems = [
  ...
  { name: 'tracker', icon: 'credit_card', label: 'Tracker', path: '/app/tracker' },
  { name: 'history', icon: 'history', label: 'History', path: '/app/history' },
  { name: 'bulletin', icon: 'newspaper', label: 'Bulletin', path: '/app/bulletin' },
